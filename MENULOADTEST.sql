...
BEGIN;
    INSERT INTO MENU ([ITEMID]
           ,[ITEMNAME]
           ,[ITEMUNITPRICE])
		   SELECT @inITEMID AS MENUID, REPLACE(@inITEMNAME,'"','') AS INAME, CONVERT(money,REPLACE(@inITEMUNITPRICE,'"','')) AS UNITPRICE
		   WHERE NOT EXISTS (SELECT 1 FROM MENU m WHERE REPLACE(@inITEMNAME,'"','') = REPLACE(m.[ITEMNAME],'"','') AND CONVERT(money,REPLACE(@inITEMUNITPRICE,'"','')) = m.[ITEMUNITPRICE]);

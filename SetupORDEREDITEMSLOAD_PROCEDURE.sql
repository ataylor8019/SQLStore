...
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.ROUTINES 
               WHERE ROUTINE_NAME = 'ORDEREDITEMSLOAD'
			   AND ROUTINE_SCHEMA = 'dbo'
			   AND ROUTINE_TYPE = 'PROCEDURE')
			       EXEC ('DROP PROCEDURE dbo.ORDEREDITEMSLOAD')
GO

CREATE PROCEDURE ORDEREDITEMSLOAD
AS
SET NOCOUNT ON
BEGIN

MERGE ORDEREDITEMS x
    USING (SELECT DISTINCT c.[ORDERID]
	                      ,b.[ITEMID]
						  ,REPLACE(a.[ITEMLINEITEMNUMBER],'"','') AS LINEITEMNUMBER 
                          ,CONVERT(NUMERIC(5,0), REPLACE(a.[ITEMQUANTITY],'"','')) AS IQUANTITY 
                          ,CONVERT(MONEY,REPLACE(a.ITEMPRICEXQUANTITY,'"','')) AS UNITPRICEXQUANTITY
           FROM IMPORT_BULKPIZZADATA a LEFT JOIN MENU b 
	       ON REPLACE(a.ITEMNAME,'"','') = b.ITEMNAME 
	       AND CONVERT(MONEY,REPLACE(a.ITEMUNITPRICE,'"','')) = b.ITEMUNITPRICE
	...
	AND x.ITEMLINEITEMNUMBER = y.LINEITEMNUMBER

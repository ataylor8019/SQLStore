...
@inFIRSTNAME, @inMIDDLEINITIAL, @inLASTNAME, @inPHONENUMBER, @inHOMEADDRESS;
...
BEGIN;
    INSERT INTO CUSTOMERS ([CUSTOMERID]
           ...
           ,[PHONENUMBER]
           ,[HOMEADDRESS])
		   SELECT REPLACE(@inCUSTOMERID,'"','') AS CUST, REPLACE(@inFIRSTNAME,'"','') AS FNAME, REPLACE(@inMIDDLEINITIAL,'"','') AS MI, 
		   REPLACE(@inLASTNAME,'"','') AS LNAME, REPLACE(@inPHONENUMBER,'"','') AS PHONE, REPLACE(@inHOMEADDRESS,'"','') AS ADDR
		   WHERE NOT EXISTS (SELECT 1 FROM CUSTOMERS c WHERE REPLACE(@inFIRSTNAME,'"','') = c.[FIRSTNAME] AND  REPLACE(@inMIDDLEINITIAL,'"','') = c.[MIDDLEINITIAL]
		   AND REPLACE(@inLASTNAME,'"','')  = c.[LASTNAME] AND REPLACE(@inPHONENUMBER,'"','') = c.[PHONENUMBER]
